      ...
      - name: Submit Vertex AI custom job แล้วรอจนจบ
        run: |
          ...
          JOB=$(gcloud ai custom-jobs create \
            ...
            --format='value(name)')
          echo "job: $JOB"
          # ไม่ใช้ stream-logs เพราะมันค้างไม่ return หลัง job จบ → step แขวนทั้งที่งานเสร็จ
          # → poll describe จนถึง terminal state แทน (log เทรนจริงดูได้ที่ Vertex console/Cloud Logging)
          for i in $(seq 1 120); do   # สูงสุด 120*30s = 60 นาที (ตรงกับ timeout-minutes)
            STATE=$(gcloud ai custom-jobs describe "$JOB" --region="$REGION" --format='value(state)')
            echo "[$i] state: $STATE"
            case "$STATE" in
              JOB_STATE_SUCCEEDED)
                echo "training succeeded"; exit 0 ;;
              JOB_STATE_FAILED|JOB_STATE_CANCELLED|JOB_STATE_EXPIRED)
                echo "training did not succeed: $STATE"
                gcloud logging read "resource.type=\"ml_job\" AND resource.labels.job_id=\"${JOB##*/}\"" \
                  --project="$PROJECT_ID" --freshness=2h --order=asc \
                  --format='value(textPayload)' 2>/dev/null | tail -80 || true
                exit 1 ;;
            esac
            sleep 30
          done
          echo "timed out waiting for Vertex job"; exit 1
